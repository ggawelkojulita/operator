...
export const displayErrorNotifications = (error, ignoreKeys = []) => {
  if (error.response !== undefined) {
    if (error.response.status === 500){
      return renderNotification('Internal server error.', 'error');
    }
    const errors = error.response.data;
    if (typeof errors === "string")
      return renderNotification('Something went wrong, error unspecified.', 'error');
    ...
    renderNotification('Something went wrong, error unspecified.', 'error')
  }
};
